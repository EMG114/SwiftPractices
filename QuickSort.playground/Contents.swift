...
func insertionSort1(inout arr:[Int], firstIdx: Int, lastIdx: Int) {
    var x, y, key: Int
...
func quickSort1(inout arr: [Int], firstIdx: Int, lastIdx: Int) {
    // base case
    if firstIdx >= lastIdx { return }
    else if lastIdx - firstIdx < 10 {
        insertionSort1(&arr, firstIdx: firstIdx, lastIdx: lastIdx)
    } else
        // partition
    { let splitPoint = partition(&arr, firstIdx: firstIdx, lastIdx: lastIdx)
        // quickSort on leftHalf
        quickSort1(&arr, firstIdx: firstIdx, lastIdx: splitPoint - 1)
        // quickSort on rightHalf
        quickSort1(&arr, firstIdx: splitPoint + 1, lastIdx: lastIdx)}
}

func quickSort1(inout arr: [Int]) {
    quickSort1(&arr, firstIdx: 0, lastIdx: arr.count-1)
}

func partition1(inout arr: [Int], firstIdx: Int, lastIdx: Int) -> Int {
    // set pivotValue to firstElement
    let pivotValue = arr[firstIdx]
    // set leftMark
    var leftMark = firstIdx + 1
...
var numbers1 = [22, 59, 38, 93,95, 0, 34, 3, 53, 6, 24, 99, 5, 636, 424, 544, 4]
