...
func insert1(x: Int, var intoSortedList xs: [Int]) -> [Int] {
    var didInsertX = false
    for i in 0..<xs.count {
        if (x < xs[i]) {
            xs.insert(x, atIndex: i)
...
func insertion_sort1(xs: [Int]) -> [Int] {
    var sortedSoFar = [Int]()
    for x in xs {
        sortedSoFar = insert(x, intoSortedList: sortedSoFar)
    }
...
func select1(xs: [Int], startingAt k: Int) -> Int {
    var minIndex = k
    for i in (k+1)..<xs.count {
        if xs[i] < xs[minIndex] {
            // we found a new minimum
...
select1([0, 1, 2, 3, 9, 5, 4, 6], startingAt: 4)
select1([8, 6, 7, 5, 3, 0, 9], startingAt: 0)

func selection_sort1(inout xs: [Int]) -> [Int] {
    for sortedSoFar in 0..<xs.count {
        let minIndex = select1(xs, startingAt: sortedSoFar)
        (xs[sortedSoFar], xs[minIndex]) = (xs[minIndex], xs[sortedSoFar])
    }
    return xs
}

var xs1 = [8, 6, 7, 5, 3, 0, 9]
selection_sort(&xs1)
